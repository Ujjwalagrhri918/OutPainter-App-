...
const Profile = () => {
  ...
  useEffect(() => {
    if (feedbackFormVisible && feedbackRef.current) {
      feedbackRef.current.measureLayout(scrollViewRef.current, (x, y) => {
...
const generateImage = async () => {
    ...
    // Construct the URL-encoded body data
    const formData = new URLSearchParams();
    formData.append('image_base64', base64Image);
    formData.append('resize_option', resizeOption); // Fixed as Full
    formData.append('alignment', alignment); // You can use the alignment state if needed
    formData.append('width', width); // User input or default based on aspect ratio
    formData.append('height', height); // User input or default based on aspect ratio
    formData.append('num_inference_steps', '8'); // Fixed
    formData.append('overlap_percentage', '1'); // Fixed
    formData.append('prompt_input', ''); // Empty value for prompt (you can add if required)
    formData.append('custom_resize_percentage', 50); // Fixed custom resize size
    ...
    try {
      ...
      // hardcored api url
      // replace with the api genrated from the fastapi here ---> "<<<<fastapi>>>/infer/"
      const response = await fetch(' https://82e7-34-75-7-75.ngrok-free.app/infer/', {
        method: 'POST',
        headers: {
      ...
      if (data.status === 'success') {
        const generatedImageBase64 = data.result_image_base64; // Get the base64 string
        setGeneratedImage(generatedImageBase64);
        setShowGeneratedImage(true);
        //console.log('Generated image base64:', generatedImageBase64);
        console.log(data);
      } else {
  ...
  const submitFeedback = async () => {
    ...
    if (
      feedbackType === "like" &&
      likeOption === "Sure, I would love to" &&
      !message
    ...
    const feedbackData = {
      email,
      message:
        feedbackType === "dislike" ? dislikeMessage + ` - ${message}` : message,
      feedbackType,
      deviceId,
      base64Image,
      generatedImage,
    ...
    const response = await createFeedback(
      ...
      feedbackData.base64Image,
      feedbackData.generatedImage
    );
  ...
  const downloadImage = async () => {
    ...
    try {
      ...
      // Check if sharing is available and trigger the download
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri);
      } else {
        Alert.alert("Download Complete", "Image saved at: " + fileUri);
      }
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Pressable onPress={imagePicker} style={styles.imageContainer}>
          {!image ? (
            <Image
              source={require("../assets/icons/uploadimg2.png")}
              style={styles.cards}
              resizeMode="contain"
            />
          ) : (
        ...
        <View style={styles.radioContainer}>
          ...
          >
            ...
            <View style={styles.radioOption}>
              ...
              <Text style={styles.radioText}>LandScape Post (16:9)</Text>
            </View>
            <View style={styles.radioOption}>
        ...
        {aspectRatio === "custom" && (
          <>
            ...
            <View style={styles.radioContainer}>
              <RadioButton.Group
                onValueChange={(newValue) => setResizeOption(newValue)}
                value={resizeOption}
              >
                <View style={styles.radioOption}>
        ...
        {!loading && showGeneratedImage && (
          <>
            ...
            {/* Save Image Button */}
            <Pressable style={styles.submitButton} onPress={downloadImage}>
                    ...
                    <View style={styles.buttonContainer}>
                      {reasons.map((reason) => (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.buttonOther,
                            dislikeReasons[reason] &&
                              styles.buttonSelectedOther, // Apply selected style conditionally
                          ]}
                        ...
                        >
                          <Text
                            style={[
                              ...
                              dislikeReasons[reason] &&
                                styles.buttonTextSelectedOther, // Apply selected text style conditionally
                            ]}
                          >
